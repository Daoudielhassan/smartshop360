...
def render_product_analysis(query_db):
    ...
    kpi = query_db(
        f'SELECT "CA" AS ca, "Notemoyenne" AS note, "QuantiteVendue" AS qte, "NbAvis" AS nb_avis '
        f'FROM v_product_kpi WHERE "ProductID" = \'{selected_id}\''
    )
    if kpi.empty:
        st.warning("Donnees insuffisantes pour ce produit.")
        return

    row = kpi.iloc[0]
    c1, c2, c3, c4 = st.columns(4)
    with c1: _kpi("CA",         f"{row['ca']:,.0f} EUR")
    with c2: _kpi("Note",       f"{row['note']:.1f} / 5")
    with c3: _kpi("Qte Vendue", f"{int(row['qte']):,}")
    with c4: _kpi("Avis",       f"{int(row['nb_avis']):,}")
    ...
    with col_left:
        st.subheader("Evolution des Ventes (6 derniers mois)")
        sales_ts = query_db(f"""
            ...
            WHERE pm."ERP_StockCode" = '{selected_id}'
              AND sf."InvoiceDate" >= NOW() - INTERVAL '6 months'
        ...
        if not sales_ts.empty:
            ...
            st.info("Pas de donnees sur les 6 derniers mois.")
    ...
    if not reviews.empty:
        for _, r in reviews.iterrows():
            icon = "pos" if r["sentiment"] == "positive" else ("neg" if r["sentiment"] == "negative" else "neu")
            st.markdown(f"**{r['rating']:.1f}/5** ({icon}): {r['txt']}")
            st.divider()
    else:
